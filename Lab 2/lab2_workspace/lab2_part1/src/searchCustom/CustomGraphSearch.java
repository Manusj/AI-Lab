...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		System.out.println("Initlizing Graphs search variables");
		...
		{
			// Poping the first node from the queue
			currentNode = PopFrontier();

			// Checking if goal is reached if goal is reached creating path to node by iterating through the parent pointers
			...
			// Finding all reachable nodes from the current node and iterating through them
			...
			for(GridPos next : reachableState)
			{
				...
				// Adding nodes to explored list and frontier if nodes is not explored
	...
	// Getter Functions for graph search

	// Function to pop the node from the front of the queue
	public SearchNode PopFrontier()
	{
		SearchNode node = frontier.removeFirst();
		return node;
	}

	// Adding node to queue in the front or back based on the variable insertFront
	public void AddNodeFrontier(SearchNode node)
	{
		if(insertFront)
	...
	// Checking if frontier empty
	public boolean IsEmptyFrontier()
	{
		return frontier.isEmpty();
	}

	// Adding nodes to explored and Frontier if it is not already present
	public void addToExploredNodes(SearchNode node)
	{
		if(!explored.contains(node))
		{
			explored.add(node);
			AddNodeFrontier(node);
		}
	}

	// Creating path from the node by iterating thorough the parents of the node.
	public ArrayList<SearchNode> createPath(SearchNode node)
	{
		while(node!=null)
		{
			path.add(0,node);
